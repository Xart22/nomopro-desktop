...
class ResourceServer extends Emitter {
  ...
  generate18nCache(locale) {
    ...
    try {
      officialTranslations = JSON.parse(
        fs.readFileSync(
          path.join(this._userDataPath, OFFICIAL_TRANSLATIONS_FILE),
          "utf8"
        )
      ); // eslint-disable-line max-len
      thirdPartyTranslations = JSON.parse(
        fs.readFileSync(
          path.join(this._userDataPath, THIRD_PARTY_TRANSLATIONS_FILE),
          "utf8"
        )
      ); // eslint-disable-line max-len
    } catch (e) {
    ...
    this.deviceIndexData[`${locale}`] = JSON.stringify(
      this.devices.assembleData(
        this._userDataPath,
        this._formatMessage[`${locale}`]
      )
    );

    this.extensionsIndexData[`${locale}`] = JSON.stringify(
      this.extensions.assembleData(
        this._userDataPath,
        this._formatMessage[`${locale}`]
      )
    );
  ...
  listen(port, host) {
    ...
    this._server
      .listen(this._port, this._host, () => {
        console.log(
          clc.green(
            `Openblock resource server start successfully, socket listen on: http://${this._host}:${this._port}`
          )
        );
        this.emit("ready");
      })
      .on("error", (err) => {
        this.isSameServer("127.0.0.1", this._port).then((isSame) => {
          if (isSame) {
            console.log(
              `Port is already used by other openblock-resource server, will try reopening after ${REOPEN_INTERVAL} ms`
            ); // eslint-disable-line max-len
            setTimeout(() => {
              this._server.close();
