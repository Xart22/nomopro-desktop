...
const AVRDUDE_STDOUT_RED_START =
  /can't open device|programmer is not responding/g;
...
class Arduino {
  constructor(peripheralPath, config, userDataPath, toolsPath, sendstd) {
    if (peripheralPath == "update") {
      ...
      const projectPathName = `${this._config.fqbn.replace(/:/g, "_")}_project`
        .split(/_/)
        .splice(0, 3)
        .join("_");
  ...
  build(code, library = []) {
    return new Promise((resolve, reject) => {
      ...
      const args = [
        ...
        this._config.fqbn,
        "--libraries",
        path.join(this._arduinoPath, "libraries"),
        "--warnings=none",
        "--verbose",
        "--build-path",
        this._buildPath,
      ...
      arduinoBuilder.stderr.on("data", (buf) => {
        ...
        if (data.search(AVRDUDE_STDERR_RED_IGNORE) !== -1) {
      ...
      arduinoBuilder.on("exit", (outCode) => {
        ...
        switch (outCode) {
          ...
          case 3:
            return reject(
              new Error("Invalid (argument for) commandline optiond"),
            );
          case 4:
            return reject(
              new Error("Preference passed to --get-pref does not exist"),
            );
          default:
    ...
    return new Promise((resolve, reject) => {
      ...
      avrdude.stderr.on("data", (buf) => {
        ...
        if (data.search(AVRDUDE_STDOUT_GREEN_START) !== -1) {
          data = this._insertStr(
            data,
            data.search(AVRDUDE_STDOUT_GREEN_START),
            ansi.green_dark,
          );
        }
        if (data.search(AVRDUDE_STDOUT_GREEN_END) !== -1) {
          data = this._insertStr(
            data,
            data.search(AVRDUDE_STDOUT_GREEN_END) + 1,
            ansi.clear,
          );
        }
        if (data.search(AVRDUDE_STDOUT_WHITE) !== -1) {
          data = this._insertStr(
            data,
            data.search(AVRDUDE_STDOUT_WHITE),
            ansi.clear,
          );
        }
        if (data.search(AVRDUDE_STDOUT_RED_START) !== -1) {
          data = this._insertStr(
            data,
            data.search(AVRDUDE_STDOUT_RED_START),
            ansi.red,
          );
        }
        this._sendstd(data);
